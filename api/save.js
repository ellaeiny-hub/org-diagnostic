export default async function handler(req, res) {
  ...
  try {
    const { manager, answers, timestamp, orgCode, questionLabels } = req.body;
    if (!manager || !answers) return res.status(400).json({ error: 'Missing fields' });
    const url = process.env.KV_REST_API_URL;
    const token = process.env.KV_REST_API_TOKEN;
    const code = orgCode || 'default';
    const id = Date.now().toString(36) + Math.random().toString(36).slice(2, 6);
    const entry = JSON.stringify({ id, manager, answers, questionLabels: questionLabels || {}, timestamp: timestamp || new Date().toISOString() });
